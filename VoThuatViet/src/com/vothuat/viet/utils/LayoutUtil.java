...
public class LayoutUtil {
	...
	public static void init(final Context pActivity) {
		...
		int width = (displayMetrics.widthPixels < displayMetrics.heightPixels) ? displayMetrics.widthPixels
				: (displayMetrics.heightPixels + getBarHeight(displayMetrics.densityDpi));
		...
		L.i(TAG, "density: " + displayMetrics.density + " width: " + width
				+ " maxWidth: + " + maxWidth);
	...
	private static void unbindViewReferences(View view) {
		...
		if (view instanceof WebView) {
			/*
			 * if(view instanceof i){ }else{ try { ((WebView) view).destroy(); }
			 * catch (Exception e) { }
			 * 
			 * }
			 */
	...
	public static void setNullImageView(ImageView imgView) {
		try {
			if (imgView != null) {
				/*
				 * Drawable drawable = imgView.getDrawable(); if (drawable
